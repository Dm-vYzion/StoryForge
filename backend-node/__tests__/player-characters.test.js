...
describe('Player characters', () => {
    it('rejects /api/player-characters/mine when unauthenticated', async () => {
        const res = await (0, supertest_1.default)(app_1.app).get('/api/player-characters/mine');
        expect([401, 403]).toContain(res.status);
        expect(res.body).toHaveProperty('success', false);
        expect(res.body).toHaveProperty('error');
    }, 10000);
    it('returns Aria characters for the logged-in user from /api/player-characters/mine', async () => {
        // Log in first
        ...
        const aria = pcs.find((pc) => pc.name === 'Aria Seed' || pc.name === 'Aria Test');
...
//# sourceMappingURL=player-characters.test.js.map